...
for link in links:
    ...
    for div in divs:
        ...
        if span.text == 'India':
            ...
            df = pd.DataFrame(list(zip(matchName,BatsmenName,Out_NotOut,Runs,Balls,Fours,Sixes,SR,matchId)),columns=['Match','Name','Out/NotOut','Runs','Balls','4s','6s','SR','MatchId'])
            ...
            df = pd.DataFrame(list(zip(Bowler_MatchName,BowlerName,Overs,Maiden,BowlRuns,Wickets,ECON,Zeroes,BowlFours,BowlSixes,Wide,NB,Bowler_MatchId)),columns=['Match','Name','Overs','Maiden','Runs','Wickets','ECON','0s','4s','6s','Wide','NB','MatchId'])
...
print(mainDfBowl)
# print(POTMdf)
mainDfBat.to_csv('FinalBattingSummaryINDIA.csv')
mainDfBowl.to_csv('FinalBowlingSummaryINDIA.csv')
