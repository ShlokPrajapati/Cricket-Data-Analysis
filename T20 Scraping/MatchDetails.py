...
for link in links:
    ...
    for i in range(len(temp_date)):
        if temp_date[i].endswith('(20-over match)'):
    ...
    T20mainSummary = pd.concat([T20mainSummary,df])
print(T20mainSummary)
T20mainSummary.to_csv('T20matchSummaryINDIA.csv')
